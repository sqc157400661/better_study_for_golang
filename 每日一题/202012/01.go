...
func (s *SliceQueue) MoveToHead(key int) {
	...
	s.Arr = append([]int{s.Arr[index]}, append(s.Arr[:index], s.Arr[index+1:]...)...)
}
...
func (l *LRUCache) Set(key, val int) {
	// 如果队列满了,要删除队尾的元素
	if l.IsFull() {
		l.hashSet.Remove(l.queue.PopQueue())
	}
	// 新节点添加到队列中
	l.queue.EnQueue(key)
	// 把新缓存的节点同时添加到hash节点中
...
func LRU(operators [][]int, k int) []int {
	lru := LRUCache{size: k,queue:&SliceQueue{Arr:[]int{}},hashSet:&Set{m:map[int]int{}}}
	var re []int
