...
--output-http-workers # 指定http输出工作池的数量 可以当成初队列的出口
--output-http-redirects  # 处理有重定向的请求，指定跟随重定向的数量
--output-http-timeout # 指定超时时间，默认：http请求和响应的超时时间为5秒
...
gor --input-raw :8001 --output-http="http://127.0.0.1:8000" --input-raw-track-response  --output-http-track-response --middleware "/website/goreplay/mid"
